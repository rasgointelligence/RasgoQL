    ...
    def wrapper(*args, **kwargs):
        ...
        if not self._dw:
            raise NotImplementedError(
                f'{func.__name__} method is only available for classes instantiated with a DW connection'
            )
        return func(*args, **kwargs)
...
def require_materialized(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        if self.table_state != TableState.IN_DW.value:
            raise NotImplementedError(
                f'{func.__name__} method is only available for tables that exist in the DataWarehouse'
            )
        return func(*args, **kwargs)
...
def require_transforms(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        if not self.transforms:
            raise NotImplementedError(
                f'{func.__name__} method is only available for SQLChains with at least one Transform'
            )
        return func(*args, **kwargs)
